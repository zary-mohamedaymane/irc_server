...
void Server::_handleCapabilityNegotiation(size_t i,
  ...
  if (tokens[0] == "LS")
    ...
    if (user._registered) {
      _sendMessage(i, ":localhost.ircserver 001 " + user._nickName +
                          " :Welcome to the 42 Internet Relay Network\
				" +
                          user._nickName + "!" + user._userName + "@" +
                          user._hostName + "\r\n");
      _sendMessage(i, ":localhost.ircserver 002 " + user._nickName +
                          " :Your host is localhost, running version 1.0\r\n");
      _sendMessage(i, ":localhost.ircserver 003 " + user._nickName +
                          " :This Sever was created today\r\n");
      _sendMessage(i, ":localhost.ircserver 004 " + user._nickName +
                          " localhost.ircserver 1.0 - itkol\r\n");
    }
  }
}
...
void Server::_handlePass(size_t i, std::vector<std::string> &tokens,
  ...
  if (tokens.empty()) {
    _sendMessage(i,
                 ":localhost.ircserver 461 * PASS :Not enough parameters\r\n");
    (_handleQuit(i, "auth error"), erased = true);
  } else if (user._registered) {
    _sendMessage(i, ":localhost.ircserver 462 " + user._nickName +
...
void Server::_handleNick(size_t i, std::vector<std::string> &tokens,
  ...
  if (tokens.empty())
    ...
    _sendMessage(i, ":localhost.ircserver 432 * " + tokens[0] +
                        " :Erroneous nickname\r\n");
  else if ((j = _getUserByNick(tokens[0])) != 0 && i != j)
    _sendMessage(i, ":localhost 433 * " + tokens[0] +
...
void Server::_handleUser(size_t i, std::vector<std::string> &tokens,
  ...
  if (tokens.size() < 4)
    _sendMessage(i, ":localhost.ircserver 461 " + user._nickName +
...
void Server::_handleJoin(size_t i, std::vector<std::string> &tokens) {
  ...
  // 2.1 channel doesnt exist -> create it (CHECK GRAMMAR!!!!) and add user to
  // operators, then add channel to server 2.2 channel exists -> check
  // double-join? -> invite-only? -> key correct? -> channel full? -> add user
  // fd to channel
  ...
  for (size_t x = 0; x < chanList.size(); x++) {
    ...
    {
      _sendMessage(i, ":localhost.ircserver 403 " + user._nickName + " " +
    ...
    if (!chanExists) {
      ...
      if (chan._inviteOnly &&
          chan._invitedFds.find(userFd) == chan._invitedFds.end()) {
        _sendMessage(i, ":localhost.ircserver 473 " + user._nickName + " " +
                            chanName + " :Cannot join channel (+i)\r\n");
        continue;
      }

      if (!chan._key.empty() && providedKey != chan._key) {
        _sendMessage(i, ":localhost.ircserver 475 " + user._nickName + " " +
                            chanName + " :Cannot join channel (+k)\r\n");
        continue;
      }

      if (chan._userLimit > 0 && chan._memberFds.size() >= chan._userLimit) {
        _sendMessage(i, ":localhost.ircserver 471 " + user._nickName + " " +
    ...
    std::string joinMsg = ":" + user._nickName + "!" + user._userName + "@" +
                          user._hostName + " JOIN :" + chanName + "\r\n";
    _broadcastToChannel(lowerChanName, joinMsg, -1);

    Channel &chan = _Channels[chanName];
    if (chan._topic.empty())
      _sendMessage(i, ":localhost.ircserver 331 " + user._nickName + " " +
                          chanName + " :No topic is set\r\n");
    else
      _sendMessage(i, ":localhost.ircserver 332 " + user._nickName + " " +
                          chanName + " :" + chan._topic + "\r\n");

    std::string nameList = "";
    for (std::set<int>::iterator mit = chan._memberFds.begin();
         mit != chan._memberFds.end(); ++mit) {
      int fd = *mit;
    ...
    _sendMessage(i, ":localhost.ircserver 353 " + user._nickName + " = " +
                        chanName + " :" + nameList + "\r\n");
    _sendMessage(i, ":localhost.ircserver 366 " + user._nickName + " " +
...
void Server::_handlePart(size_t i, std::vector<std::string> &tokens) {
  ...
  // 2.1 channel exists?
  // 2.2 user in channel?
  // 2.3 remove user from channel and broadcast part message to channel +
  // leaving user 2.4 if channel is empty, remove it
  ...
  for (size_t x = 0; x < chanList.size(); x++) {
    ...
    bool chanExists = (_Channels.find(lowerChanName) != _Channels.end());
    if (!chanExists) {
      _sendMessage(i, ":localhost.ircserver 403 " + chanName +
                          " :No such channel\r\n");
      continue;
    }
    ...
    Channel &chan = _Channels[lowerChanName];
    if (chan._memberFds.find(userFd) == chan._memberFds.end()) {
      _sendMessage(i, ":localhost.ircserver 442 " + chanName +
                          " :You're not on that channel\r\n");
      continue;
    }

    std::string partMsg = ":" + user._nickName + "!" + user._userName + "@" +
                          user._hostName + " PART :" + chanName + "\r\n";
...
void Server::_handlePrivmsg(size_t i, std::vector<std::string> &tokens) {
  ...
  // 2.1 target is channel -> check channel exists? -> user is channel member?
  // -> send to members expect sender 2.2 target is a user -> check user
  // existence? -> send to user

  int userFd = _pollFds[i].fd;
  User &user = _Users[userFd];

  if (tokens.empty()) {
    _sendMessage(i, ":localhost.ircserver 411 " + user._nickName +
                        " :No recipient given (PRIVMSG)\r\n");
    return;
  }
  if (tokens.size() < 2 || tokens[1].empty()) {
    _sendMessage(i, ":localhost.ircserver 412 " + user._nickName +
                        " :No text to send\r\n");
    return;
  }
  ...
  for (size_t t = 0; t < targets.size(); t++) {
    ...
    std::string msgPrefix = ":" + user._nickName + "!" + user._userName + "@" +
                            user._hostName + " PRIVMSG " + target + " :" +
                            messageText + "\r\n";

    if (target[0] == '#' || target[0] == '&') {
      std::string lowerChanName = _tolowerStr(target);
      if (_Channels.find(lowerChanName) == _Channels.end()) {
        _sendMessage(i, ":localhost.ircserver 401 " + user._nickName + " " +
      ...
      if (chan._memberFds.find(userFd) == chan._memberFds.end()) {
        _sendMessage(i, ":localhost.ircserver 404 " + user._nickName + " " +
